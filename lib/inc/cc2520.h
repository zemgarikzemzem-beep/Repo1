...
#define CC_RES 		GPIOA->BRR|=(1<<1)
#define CC_START 	GPIOA->BSRR|=(1<<1)
#define CC_CSEN 	GPIOA->BRR|=(1<<4)
#define CC_CSDIS 	GPIOA->BSRR|=(1<<4)
...
#define RF_TXPOWER_MIN_18_DBM           	1
#define RF_TXPOWER_MIN_7_DBM		     			2
#define RF_TXPOWER_MIN_4_DBM    		 			3
#define RF_TXPOWER_MIN_2_DBM     					4
#define RF_TXPOWER_0_DBM	     						5
#define RF_TXPOWER_1_DBM	     						6
#define RF_TXPOWER_2_DBM	     						7
#define RF_TXPOWER_3_DBM	     						8
#define RF_TXPOWER_4_DBM	     						9

#define CC2520_RSSI_OFFSET                76  // dBm (from data sheet)
#define CC2520_MDMCTRL0_VAL 							0x85
...
#define CC2520_BEGINS_OF_TRANSMISSION			192	//192
...
#define FOOTER_LEN												2
#define FOOTER_CRC_OK      								0x80
#define FOOTER_CORRELATION 								0x7f
...
#define CC2520_EXC_SFD                 		13		// 0x0D 0000.1101
...
#define CC2520_EXT_CLOCK_1_00MHZ       		32
#define CC2520_EXT_CLOCK_1_03MHZ       		33
#define CC2520_EXT_CLOCK_1_07MHZ          34
#define CC2520_EXT_CLOCK_1_10MHZ       		35
#define CC2520_EXT_CLOCK_1_14MHZ       		36
#define CC2520_EXT_CLOCK_1_19MHZ       		37
#define CC2520_EXT_CLOCK_1_23MHZ       		38
#define CC2520_EXT_CLOCK_1_28MHZ       		39
#define CC2520_EXT_CLOCK_1_33MHZ       		40
#define CC2520_EXT_CLOCK_1_39MHZ       		41
#define CC2520_EXT_CLOCK_1_45MHZ       		42
#define CC2520_EXT_CLOCK_1_52MHZ       		43
#define CC2520_EXT_CLOCK_1_60MHZ       		44
#define CC2520_EXT_CLOCK_1_68MHZ       		45
#define CC2520_EXT_CLOCK_1_78MHZ       		46
#define CC2520_EXT_CLOCK_1_88MHZ       		47
#define CC2520_EXT_CLOCK_2_00MHZ       		48
#define CC2520_EXT_CLOCK_2_13MHZ       		49
#define CC2520_EXT_CLOCK_2_29MHZ       		50
#define CC2520_EXT_CLOCK_2_46MHZ       		51
#define CC2520_EXT_CLOCK_2_67MHZ       		52
#define CC2520_EXT_CLOCK_2_91MHZ       		53
#define CC2520_EXT_CLOCK_3_20MHZ       		54
#define CC2520_EXT_CLOCK_3_56MHZ       		55
#define CC2520_EXT_CLOCK_4_00MHZ       		56
#define CC2520_EXT_CLOCK_4_57MHZ       		57
#define CC2520_EXT_CLOCK_5_33MHZ       		58
#define CC2520_EXT_CLOCK_6_40MHZ       		59
#define CC2520_EXT_CLOCK_8_00MHZ       		60
#define CC2520_EXT_CLOCK_10_67MHZ      		61
#define CC2520_EXT_CLOCK_16_00MHZ      		62

// Register bits
// Register FRMFILT0, bit = 1 enabled frame filtering 
#define CC2520_FRAME_FILTER_EN						0
// Register FRMCTRL0, bit = 1 enabled autoCRC
#define CC2520_AUTOCRC										(1 << 6)

#define CC2520_MAKE_BIT_ADDR(regAddr, bitIndex) (((regAddr) << 3) | (bitIndex))

// Определяется специально для каждой платы
#define CC2520_TEST_FIFOP1								(GPIOB->IDR & GPIO_Pin_0)
#define CC2520_TEST_FIFOP2								(GPIOB->IDR & GPIO_Pin_11)

extern volatile uint8_t prg_en;
extern uint16_t label;

void CC2520_ins_wr_array(uint16_t, uint8_t *);
uint8_t CC2520_ins_strobe(uint8_t);
uint8_t CC2520_get_status_byte(void);
uint8_t CC2520_ssamplecca(void);
uint8_t CC2520_sxosccon(void);
uint8_t CC2520_stxcal(void);
uint8_t CC2520_srxon(void);
uint8_t CC2520_srxon_low_current(void);
...
uint8_t CC2520_sreset(void);
uint8_t CC2520_memrd(uint16_t, uint16_t, uint8_t *);
uint8_t CC2520_memrd8(uint16_t);
uint8_t CC2520_memwr(uint16_t, uint16_t, uint8_t *);
uint8_t CC2520_memwr8(uint16_t, uint8_t);
uint8_t CC2520_memwr16(uint16_t, uint16_t);
uint8_t CC2520_read_rxbuf(uint8_t, uint8_t *);
uint8_t CC2520_rxbufcp_begin(uint16_t, uint8_t *);
uint8_t CC2520_rxbufcp_end(uint16_t, uint8_t, uint8_t *);
uint8_t CC2520_rxbufmov(uint8_t, uint16_t, uint8_t, uint8_t *);
uint8_t CC2520_write_txbuf(uint8_t, uint8_t *);
uint8_t CC2520_txbufcp(uint8_t, uint16_t, uint8_t, uint8_t *);
uint8_t CC2520_random(uint8_t, uint8_t *);
uint8_t CC2520_random8(void);
uint16_t CC2520_random16(void);
uint8_t CC2520_bset(uint8_t);
uint8_t CC2520_bclr(uint8_t);
uint8_t CC2520_regrd(uint8_t, uint8_t, uint8_t *);
uint8_t CC2520_regrd8(uint8_t);
uint16_t CC2520_regrd16(uint8_t);
uint8_t CC2520_regwr(uint8_t, uint8_t, uint8_t *);
void CC2520_regwr8(uint8_t, uint8_t);
uint8_t CC2520_get_exception(uint8_t);
uint8_t CC2520_wait_exception(uint8_t, uint16_t);
void CC2520_clear_exception(uint8_t);
uint8_t CC2520_get_radio_status(uint16_t);
void CC2520_set_channel(uint8_t);
void CC2520_set_power(uint8_t);
void CC2520_set_ext_clock(uint8_t);
uint8_t CC2520_init(void);
void tranceiver_init(void);
void tranceiver_powerdown(void);
uint8_t CC2520_tx_from_idle(void);
uint8_t CC2520_tx_from_rx(void);
void CC2520_send_ansver(void);
uint8_t CC2520_receive(uint8_t *);
