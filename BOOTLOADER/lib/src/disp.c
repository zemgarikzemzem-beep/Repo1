...
void TFT_Init(void){
  ...
  delay(5);
	...
	TFT_CSEN;
  TFT_Send_Command(ST7735_SWRESET); // –есет
  delay(150);
  ...
  TFT_Send_Command(ST7735_MADCTL); // This command defines read/ write scanning direction of frame memory (Ќ”∆Ќќ Ќ≈ RGB, a BGR!!!)
  TFT_Send_Data(0xC8); // BGR и перевЄрнутый дисплей
  TFT_Send_Command(ST7735_COLMOD); // ÷ветовой режим
  ...
  TFT_Send_Data(0x0E);
  TFT_Send_Command(ST7735_GMCTRP1); // Gamma (С+Тpolarity) Correction Characteristics Setting
  TFT_Send_Data(0x02);
  TFT_Send_Data(0x1c);
  TFT_Send_Data(0x07);
  TFT_Send_Data(0x12);
  ...
  TFT_Send_Data(0x03);
  TFT_Send_Data(0x10);
  TFT_Send_Command(ST7735_GMCTRN1); // Gamma С-Тpolarity Correction Characteristics Setting
  TFT_Send_Data(0x03);
  TFT_Send_Data(0x1d);
  TFT_Send_Data(0x07);
  TFT_Send_Data(0x06);
  ...
  delay(10);
  TFT_Send_Command(ST7735_DISPON); // ¬ключаем!
  delay(100);
